...
from evacode.config import ConfigError, load_config
from evacode.hooks import HookConfigError, HookEngine, load_hooks
from evacode.permissions import PermissionMode


def main() -> None:
    # 先确保 .evacode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".evacode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".evacode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="evacode", description="EvaCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from evacode.remote import RemoteServer
    ...
    from evacode.app import EvaCodeApp
    from evacode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from evacode.client import create_client, resolve_context_window
    from evacode.conversation import ConversationManager
    from evacode.memory.instructions import load_instructions
