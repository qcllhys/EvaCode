...
from evacode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from evacode.conversation import ThinkingBlock as ConvThinkingBlock
from evacode.memory.auto_memory import MemoryManager
from evacode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from evacode.hooks import HookContext, HookEngine, ToolRejectedError
from evacode.hooks.engine import HookNotification
from evacode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from evacode.tools import ToolRegistry
from evacode.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    def __init__(
        ...
        if memory_manager is not None:
            from evacode.memory.consolidation import MemoryConsolidator
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".evacode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".evacode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from evacode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from evacode.context.manager import (
