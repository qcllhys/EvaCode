...
from evacode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from evacode.commands.completion import CompletionPopup
from evacode.commands.handlers import register_all_commands
from evacode.config import MCPServerConfig, ProviderConfig
from evacode.hooks import HookContext, HookEngine, load_hooks
from evacode.conversation import ConversationManager, Message
from evacode.mcp import ConnectResult, MCPManager
from evacode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from evacode.agents.task_manager import TaskManager
from evacode.agents.trace import TraceManager
from evacode.agents.notification import inject_task_notifications
from evacode.commands.handlers.tasks import create_tasks_command
from evacode.skills.executor import SkillExecutor
from evacode.skills.loader import SkillLoader
from evacode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from evacode.cache import FileCache
from evacode.tools import ToolRegistry, create_default_registry
from evacode.tools.agent_tool import AgentTool
from evacode.tools.ask_user import AskUserEvent, AskUserTool
from evacode.tools.impl.tool_search import ToolSearchTool
from evacode.tools.install_skill import InstallSkillTool
from evacode.tools.load_skill import LoadSkill
from evacode.worktree.cleanup import start_stale_cleanup_task
from evacode.worktree.manager import WorktreeManager
from evacode.commands.handlers.worktree import create_worktree_command
from evacode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".evacode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".evacode" / "history"
...
class EvaCodeApp(App):
    ...
    def __init__(
        ...
        from evacode.config import SandboxAppConfig
    ...
    def on_mount(self) -> None:
        self.register_theme(_EVACODE_THEME)
        self.theme = "evacode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".evacode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".evacode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".evacode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from evacode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.evacode/config.yaml",
                        f"{work_dir}/.evacode/permissions.local.yaml",
                    ],
        ...
        from evacode.filehistory import FileHistory
        ...
        from evacode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from evacode.config import WorktreeConfig
        ...
        from evacode.tools.enter_worktree import EnterWorktreeTool
        from evacode.tools.exit_worktree import ExitWorktreeTool
        ...
        from evacode.tools.team_create import TeamCreateTool
        from evacode.tools.team_delete import TeamDeleteTool
        ...
        from evacode.commands.handlers.trace import create_trace_command
        ...
        from evacode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _show_plan_approval(self) -> None:
        from evacode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from evacode.plan_dialog import InlinePlanWidget, PlanChoice
        from evacode.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from evacode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from evacode.permission_dialog import InlinePermissionWidget
