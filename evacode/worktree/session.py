...
def _session_path(evacode_dir: Path) -> Path:
    return evacode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(evacode_dir)
    if session is None:
...
def load_worktree_session(evacode_dir: Path) -> WorktreeSession | None:
    path = _session_path(evacode_dir)
    if not path.exists():
        return None
    try:
