...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".evacode" / "config.yaml",
        cwd / ".evacode" / "config.yaml",
        cwd / ".evacode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .evacode/config.yaml "
            "in project or ~/.evacode/config.yaml"
        )
    return merged
