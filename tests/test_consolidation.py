...
需要环境变量 EVACODE_TEST_API_KEY、EVACODE_TEST_BASE_URL、EVACODE_TEST_MODEL。
...
def test_lock_first_acquire():
    from evacode.memory.consolidation import _read_last_consolidated_at, _try_acquire_lock
...
def test_lock_blocks_when_held():
    from evacode.memory.consolidation import _try_acquire_lock
...
def test_lock_rollback_deletes_on_zero():
    from evacode.memory.consolidation import _try_acquire_lock, _rollback_lock
...
def test_prompt_contains_all_phases():
    from evacode.memory.consolidation import _build_consolidation_prompt
...
@pytest.mark.skipif(
    not os.environ.get("EVACODE_TEST_API_KEY"),
    reason="EVACODE_TEST_API_KEY not set"
)
def test_e2e_consolidation_merges_duplicates():
    api_key = os.environ["EVACODE_TEST_API_KEY"]
    base_url = os.environ.get("EVACODE_TEST_BASE_URL", "https://api.example.com/v1")
    model = os.environ.get("EVACODE_TEST_MODEL", "test-model")

    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".evacode", "memory")
...
async def _run_consolidation(work_dir, api_key, base_url, model, mem_dir):
    ...
    from evacode.permissions.checker import PermissionChecker
    from evacode.tools import ToolRegistry
    from evacode.tools.bash import Bash
    from evacode.tools.edit_file import EditFile
    from evacode.tools.glob import Glob
    from evacode.tools.grep import Grep
    from evacode.tools.read_file import ReadFile
    from evacode.tools.write_file import WriteFile

    from evacode.config import ProviderConfig
    from evacode.client import OpenAICompatClient
    ...
    from evacode.permissions.rules import RuleEngine
    from evacode.permissions.dangerous import DangerousCommandDetector
    from evacode.permissions.checker import PermissionMode
